...
@section('titulo') Index de paises
@endsection
...
<div class="row">
	...
	<div class="col-xl-1 col-lg-2 col-md-2 col-sm-12 col-xs-12">
		<a href="{{url('/paises')}}/create" class="btn btn-outline-success col-sm-12 col-xs-12">
...
<div class="table-responsive table-responsive-xl table-responsive-md table-responsive-lg table-responsive-sm">
	...
	<table class="table table-hover table-sm ">
		...
		<tbody>
			...
			<tr>
				<th scope="row" class="text-center align-middle">{{$item ->id_pais}}</td>
					<td class="text-center align-middle">{{$item ->nombre}}</td>
					<td class="text-center align-middle ">
						<a href="{{URL::action('PaisesController@edit',$item ->id_pais)}}">
                            <img length="30px" width="30px" src="{{asset('imgs/editar.svg')}}" title="Editar"></img>
                        </a>
						<a href="" data-target="#modal-delete-{{$item->id_pais}}" data-toggle="modal">
                             <img length="30px" width="30px" src="{{asset('imgs/eliminar.svg')}}" title="Eliminar"></img>
                        </a>
					</td>
			</tr>
	@include('paises.modal') @endforeach
		</tbody>
	</table>
	{{--$paiseses->links()--}}
	@include('paises.paginador')
</div>
@endsection
